...
Thumbnail.prototype.resize = function (image, next) {
  ...
  gm(image.Body).size(function(err, val){
    if (val.width > _this.options.width || 
      ...
      // next directly
      next(null, image.Body, image.ContentType)
    }
  })
...
Thumbnail.prototype._applyResize = function(image, size, next) {
  ...
  const height = Math.min(this.options.height, size.height)
  gm(image.Body)
    .crop(width, height, Math.round(Math.abs(this.options.width -size.width) / 2), 0)
