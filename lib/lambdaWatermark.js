...
var lambdaWrapper = function (options) {
  ...
  return function (event, context) {
    ...
    function uploadWatermark(image, next) {
      ...
      s3.putObject({
        Bucket: dstBucket,
        Key: dstKey.split(".")[0] + `_(${image.size.width}*${image.size.height}).` + dstKey.split(".")[1],
        Body: image.Body,
    ...
    return s3.getObject({
      ...
      if (err) {
        ...
        gm(image.Body).autoOrient().toBuffer(function (err, buffer) {
          if (err) {
            log.error(err)
          } else {
            const orintedImage = {
            ...
            const rawFlow = [
              (next) => {
                watermarker.watermark.bind(watermarker)(orintedImage, next)
              },
              uploadWatermark
            ];
            const thumbnailFlow = [
              (next) => {
                thumbnail.resize.bind(thumbnail)(orintedImage, next)
              },
              // add watermark
