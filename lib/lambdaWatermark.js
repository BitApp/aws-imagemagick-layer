...
var lambdaWrapper = function (options) {
  ...
  return function (event, context) {
    ...
    function uploadWatermark(image, next) {
      ...
      s3.putObject({
        Bucket: dstBucket,
        Key: dstKey.split(".")[0] + `(${image.size.width}x${image.size.height}).` + dstKey.split(".")[1],
        Body: image.Body,
    ...
    function uploadThumbnail(image, next) {
      ...
      s3.putObject({
        Bucket: dstBucket,
        Key: dstKey.split(".")[0] + `-thumbnail(${image.size.width}x${image.size.height}).` + dstKey.split(".")[1],
        Body: image.Body,
        ContentType: image.ContentType
      }, (err) => {
        next(err, {
    ...
    function uploadGlass(image, next) {
      ...
      s3.putObject({
        Bucket: dstBucket,
        Key: dstKey.split(".")[0] + `-glass(${image.size.width}x${image.size.height}).` + dstKey.split(".")[1],
        Body: image.Body,
        ContentType: image.ContentType
      }, next);
    }
