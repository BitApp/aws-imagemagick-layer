...
RUN yum install -y libpng-devel libjpeg-devel libwebp-tools libglvnd-glx libXi fontconfig-devel wget gcc tar
...
RUN mkdir layer && mkdir layer/bin && cp /var/task/graphicsmagick/bin/gm ./layer/bin && cp /var/task/imagemagick/bin/convert ./layer/bin && cp /var/task/imagemagick/bin/magick ./layer/bin && cp /var/task/imagemagick/bin/stream ./layer/bin && cp /var/task/imagemagick/bin/identify ./layer/bin
RUN mkdir layer/sharedlib && cp /lib64/libjpeg.so.62 ./layer/sharedlib && cp /lib64/libpng15.so.15 ./layer/sharedlib && cp /lib64/libgomp.so.1 ./layer/sharedlib && cp /lib64/libfontconfig.so.1 ./layer/sharedlib && cp /lib64/libfreetype.so.6 ./layer/sharedlib
